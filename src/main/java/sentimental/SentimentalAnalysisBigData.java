...
//	   }
		...
		JavaRDD<String> data2 = spark.sparkContext().textFile("data/sa/test2.txt", 1).toJavaRDD();

		JavaRDD<TweetVO> tweetsRdd2 = data2.map(strRow -> {
			//System.out.println(strRow);
			String[] rowArr = strRow.split("\t");
			String rawTweet = rowArr[1];
			String realTweet = rawTweet.replaceAll(",", "").replaceAll("\"", "").replaceAll("\\*", "").replaceAll("\\.", "").trim();
		...
		Dataset<Row> tweetsDs2 = spark.createDataFrame(tweetsRdd2.rdd(), TweetVO.class);
		Dataset<Row> updTweetsDS2 = pm.transform(tweetsDs2);
		updTweetsDS2.show();
